class CoordinateSystem {
  ...
  registerEventListeners(element, { xOnly=false, yOnly=false } = {}) {
    // Pan
    ...
    document.addEventListener('pointermove', (event) => {
      ...
      // This is not a move event beginning from the element this listener
      // was attached to
      if (this.startingElement !== element) {
        return
      }
      ...
      if (this.mousedown && this.prevMouseX && this.prevMouseY) {
        ...
        if (!yOnly) this.originX += (event.clientX - this.prevMouseX)
        if (!xOnly) this.originY += (event.clientY - this.prevMouseY)
        this.emitChange()
      }
